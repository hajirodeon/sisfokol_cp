...
//nilai
$filenya = "g_video.php";
$judul = "[INTERAKTIF]. Galeri Video";
$judulku = "[INTERAKTIF]. $judul";
$judulx = $judul;
$s = nosql($_REQUEST['s']);
...
if($_POST['btnSMP'])
	...
	if ((empty($e_judul)) OR (empty($e_urlnya)))
		...
		if ((empty($s)) OR ($s == "baru"))
			...
			//query
			mysqli_query($koneksi, "INSERT INTO cp_g_video(kd, judul, filex, postdate) VALUES ".
			...
			{
			//query
			mysqli_query($koneksi, "UPDATE cp_g_video SET judul = '$e_judul', ".
							"filex = '$e_urlnya', ".
							"postdate = '$today' ".
							"WHERE kd = '$kdku'");
...
if($_POST['btnHPS'])
	...
	for ($i=1; $i<=$limit;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM cp_g_video ".
						"WHERE kd = '$kd'");
...
if (($s == 'baru') OR ($s == 'edit'))
	{
	//query
	$qx = mysqli_query($koneksi, "SELECT * FROM cp_g_video ".
						"WHERE kd = '$kdku'");
	...
	URL VIDEO YOUTUBE : 
	<br>
	<input name="e_urlnya" id="e_urlnya" type="text" value="'.$e_filex.'" size="50" class="btn btn-warning">
	</p>

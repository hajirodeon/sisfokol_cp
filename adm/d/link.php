...
if ($_POST['btnSMP'])
	...
	if ((empty($e_nama)) OR (empty($e_urlnya)))
		...
		if ($s == "baru")
			{
			//insert
			mysqli_query($koneksi, "INSERT INTO cp_m_link(kd, nama, filex, urlnya, postdate) VALUES ".
		...
		if ($s == "edit")
			{
			mysqli_query($koneksi, "UPDATE cp_m_link SET nama = '$e_nama', ".
							"urlnya = '$e_urlnya', ".
							"filex = '$namabaru', ".
							"postdate = '$today' ".
							"WHERE kd = '$e_kd'");
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM cp_m_link ".
						"WHERE kd = '$kd'");
		}
...
if (($s == "baru") OR ($s == "edit"))
	{
	//edit
	$qx = mysqli_query($koneksi, "SELECT * FROM cp_m_link ".
						"WHERE kd = '$kd'");
	...
	if (empty($e_filex1))
		...
		{
		$nil_foto = "$sumber/filebox/link/$e_kd/$e_filex1";
		}
	...
	$(document).ready(function(){
	       ...
	       $('#image-holder').load("<?php echo $sumber;?>/adm/d/i_link.php?aksi=lihat1&kd=<?php echo $kd;?>");
	    ...
	    $('#upload_image').on('change', function(event){
	     event.preventDefault();
			...
			$('#loading').show();
		     ...
		     $.ajax({
		      url:"i_link_upload.php?kd=<?php echo $kd;?>",
		      method:"POST",
		      ...
		      success:function(data){
				$('#loading').hide();
		       $('#preview').load("<?php echo $sumber;?>/adm/d/i_link.php?aksi=lihat&kd=<?php echo $kd;?>");
	...
	$sqlcount = "SELECT * FROM cp_m_link ".
	...
	if ($count != 0)
		...
		do {
			...
			<td>
			<p>
			<img src="'.$sumber.'/filebox/link/'.$e_kd.'/'.$filex1.'" width="100%" height="100">
			</p>

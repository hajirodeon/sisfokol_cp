...
//nilai
$filenya = "mitra.php";
$judul = "[MASTER] Data Mitra/Partner";
$judulku = "$judul";
$judulx = $judul;
$s = nosql($_REQUEST['s']);
...
if ($_POST['btnSMP'])
	...
	//nek null
	if ((empty($e_nama)) OR (empty($e_urlnya)))
		{
		//re-direct
		...
		if ($s == "baru")
			{
			//insert
			mysqli_query($koneksi, "INSERT INTO cp_m_mitra(kd, nama, filex, urlnya, postdate) VALUES ".
							"('$e_kd', '$e_nama', '$namabaru', '$e_urlnya', '$today')");
		...
		if ($s == "edit")
			{
			mysqli_query($koneksi, "UPDATE cp_m_mitra SET nama = '$e_nama', ".
							"urlnya = '$e_urlnya', ".
							"filex = '$namabaru', ".
							"postdate = '$today' ".
							"WHERE kd = '$e_kd'");
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM cp_m_mitra ".
						"WHERE kd = '$kd'");
		}
...
if (($s == "baru") OR ($s == "edit"))
	{
	//edit
	$qx = mysqli_query($koneksi, "SELECT * FROM cp_m_mitra ".
						"WHERE kd = '$kd'");
	...
	if (empty($e_filex1))
		...
		{
		$nil_foto = "$sumber/filebox/mitra/$e_kd/$e_filex1";
		}
	...
	http://<input name="e_urlnya" type="text" size="30" value="'.$e_urlnya.'" class="btn btn-warning">
	</p>
	...
	$(document).ready(function(){
	       ...
	       $('#image-holder').load("<?php echo $sumber;?>/adm/d/i_mitra.php?aksi=lihat1&kd=<?php echo $kd;?>");
	    ...
	    $('#upload_image').on('change', function(event){
	     event.preventDefault();
			...
			$('#loading').show();
		     ...
		     $.ajax({
		      url:"i_mitra_upload.php?kd=<?php echo $kd;?>",
		      method:"POST",
		      ...
		      success:function(data){
				$('#loading').hide();
		       $('#preview').load("<?php echo $sumber;?>/adm/d/i_mitra.php?aksi=lihat&kd=<?php echo $kd;?>");
	...
	<?php	
	echo '<p>
	NB. File Image dengan Resolusi 200 x 200 pixel
	</p>
	...
	$sqlcount = "SELECT * FROM cp_m_mitra ".
	...
	<td><strong><font color="'.$warnatext.'">URL</font></strong></td>
	...
	if ($count != 0)
		...
		do {
			...
			<td>
			<p>
			<img src="'.$sumber.'/filebox/mitra/'.$e_kd.'/'.$filex1.'" width="100%" height="100">
			</p>

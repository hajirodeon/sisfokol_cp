...
if ($_POST['btnSMP'])
	...
	if ((empty($e_nama)) OR (empty($e_urlnya)))
		...
		if ($s == "baru")
			{
			//insert
			mysql_query("INSERT INTO cp_m_mitra(kd, nama, filex, urlnya, postdate) VALUES ".
		...
		if ($s == "edit")
			{
			mysql_query("UPDATE cp_m_mitra SET nama = '$e_nama', ".
							"urlnya = '$e_urlnya', ".
							"filex = '$namabaru', ".
							"postdate = '$today' ".
							"WHERE kd = '$e_kd'");
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysql_query("DELETE FROM cp_m_mitra ".
						"WHERE kd = '$kd'");
		}
...
if (($s == "baru") OR ($s == "edit"))
	{
	//edit
	$qx = mysql_query("SELECT * FROM cp_m_mitra ".
						"WHERE kd = '$kd'");
